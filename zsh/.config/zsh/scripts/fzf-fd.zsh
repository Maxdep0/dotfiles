...
NEOVIM_EXTENSIONS="txt|md|html|css|js|ts|json|xml|yaml|yml|csv|py|rb|sh|zsh|vim|lua|java|c|cpp|h|hpp|cs|php|pl|go|rs|swift|sql|toml|ini|conf|cfg|log"
...
if [[ -f /tmp/fzf-fd-select ]]; then
    ...
    if [[ -d "$SELECTED_FILE" ]]; then
        ...
        cd "$HOME/${SELECTED_FILE%/*}" && nvim ${SELECTED_FILE##*/}
